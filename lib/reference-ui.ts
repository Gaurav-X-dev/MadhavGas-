...
function renderAchievementsSection(data: PublicSiteData) {
  ...
  const glyphs = ['&#9733;', '&#10003;', '&#9670;', '&#10022;', '&#9673;'];
  return `<section class="section achievements-section"><div class="container"><div class="section-head"><div><span class="eyebrow">Shared Progress</span><h2>Achievements Across MBGA &amp; Bharatgas</h2></div><p class="lead">Published entries document factual service progress and brand relationships without implying unverified awards.</p></div>${items.length ? `<div class="awards-grid stagger-group">${items.map((item, index) => `<article class="award-card${index === 0 ? ' award-card-featured' : ''} reveal-up"><span class="award-year">${escapeHtml(item.brand)}</span>${item.imageUrl ? `<img class="award-card-image" src="${escapeHtml(item.imageUrl)}" alt="${escapeHtml(item.title)}" loading="lazy" />` : `<div class="award-icon">${glyphs[index % glyphs.length]}</div>`}<small>${escapeHtml(item.type)}</small><h3>${escapeHtml(item.title)}</h3><p>${escapeHtml(item.description)}</p><span class="award-label">${escapeHtml(item.year)}</span></article>`).join('')}</div>` : '<div class="content-empty-state">No achievements are currently published.</div>'}</div></section>`;
}
