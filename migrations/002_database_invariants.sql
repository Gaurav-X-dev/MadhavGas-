...
DO $$ BEGIN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'cms_singletons_key_check') THEN
    ALTER TABLE cms_singletons ADD CONSTRAINT cms_singletons_key_check
      CHECK (key IN ('site-content','sustainability','qr-settings','agency-settings'));
  END IF;
